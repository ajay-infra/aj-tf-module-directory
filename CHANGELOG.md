...
- `skills.md`'s "Stable ref" pointed at `github.com/ajaylakma/aj-tf-module-directory?ref=directory-01` — wrong org (real org is `ajay-infra`) and a branch that doesn't exist (only `main` — confirmed via `git branch -a`; no tags existed either, despite `README.md`'s own Usage example already correctly referencing `?ref=v0.1.0`, also nonexistent). Fixed both refs to `v1.0.0` and cut that tag (module was fully implemented with no prior release).
...
Initial release — AWS Managed Microsoft AD in the management VPC, Secrets-Manager-backed admin credentials, AD security group, DHCP options wiring. Module was already fully implemented; this tag just formalizes the first stable release so `README.md`/`skills.md` have something real to pin to.
